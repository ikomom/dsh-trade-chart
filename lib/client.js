...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function KlineChart(props) {
			...
			// 横向虚线 + y 轴价格标签：价格面板内跟随鼠标价格；面板外（副图/横轴）显示当前 K 线收盘价，
			// 保证任何 hover 位置都能读到价格（与 y 轴刻度同一方向）
			if (hover !== null && hoverVi !== null) {
				const inPrice = hover.py >= T && hover.py <= T + priceH;
				const dispPrice = inPrice && typeof hover.price === 'number' ? hover.price : kline[hoverVi].close;
				const hpy = y(dispPrice);
				els.push(lineEl('hxl', L, hpy, W - R, hpy, { stroke: 'rgba(128,128,128,0.5)', strokeWidth: 1, strokeDasharray: '3 3' }));
				const priceText = fmtPrice(dispPrice) + unit;
				const pyy = clamp(hpy, T + 10, T + priceH - 6);
				// 放在 y 轴刻度区（右缘），水平左对齐，与刻度数字同方向；高亮背景条 + 红色描边
				...
				els.push(React.createElement('text', { key: 'hxt', x: W - R + 8, y: pyy + 4, fontSize: 11, fontWeight: '600', fill: 'var(--dsw-alias-label-primary)', textAnchor: 'start' }, priceText));
			}
