...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CARD_CSS = [
			...
			'.tc-chart{display:block;width:100%;height:auto}',
		...
		function KlineChart(props) {
			...
			const onMove = (e) => {
				...
				if (pan !== null) {
					const delta = pan.anchorRaw - d.rawI;
					const newStart = clamp(pan.startRaw + delta, 0, origN - pan.len);
			...
			const onDown = (e) => {
				...
				if (d === null) return;
				if (tool === 'select') {
					setPan({ startRaw: vStart, len: wLen, anchorRaw: d.rawI });
					return;
				}
