	...
	factory: (require) => {
		...
		function KlineChart(props) {
			...
			const periods = Array.isArray(spec.ma) && spec.ma.length > 0
				...
				: [5, 10, 20];
			// MA 基于全量数据计算（缩放/平移后均线仍连续正确，窗口边缘不缺历史）
			...
			const onDoubleClick = () => {
				if (tool === 'select') { setView(null); setYLock(null); }
			};
			const onCopy = () => {
				const pt = (p) => {
					const o = {};
			...
			maSeries.forEach((ms) => {
				// 全量 MA 值按窗口映射：缩放后均线与全览时该区间完全一致，边缘不缺历史
				let d = '';
				let started = false;
				for (let i = 0; i < n; i++) {
					const v = ms.vals[maRawAt(i)];
				...
				els.push(React.createElement('path', { key: 'ma' + ms.period, d, fill: 'none', stroke: ms.color, strokeWidth: 1.5, strokeLinejoin: 'round' }));
			});
			// 标注（模型 + 自动波段 + 手绘 + 绘制预览）
			...
			const footer = React.createElement('div', { className: 'tc-last' }, [
				...
				zoomed ? React.createElement('span', { key: 'f3' }, '  ·  显示 ' + shortTime(wData[0].time) + ' ~ ' + shortTime(wData[wLen - 1].time) + '（' + wLen + '/' + origN + ' 根，滚轮缩放/拖动平移，上下=价格）') : null,
			...
			const tools = React.createElement('div', { className: 'tc-tools' }, [
				...
				React.createElement('button', {
					...
					onClick: () => { setView(null); setYLock(null); },
				}, '⟲'),
				React.createElement('button', {
